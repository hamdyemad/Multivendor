...
@section('content')
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-25">
                        ...
                        @can('roles.create')
                            <div class="d-flex gap-2">
                                <a href="{{ route('admin.vendor-users-management.roles.create') }}"
    ...
    <x-delete-modal modalId="modal-delete-role" :title="trans('roles.confirm_delete')" :message="trans('roles.delete_warning')" itemNameId="delete-role-name"
        confirmBtnId="confirmDeleteRoleBtn" :deleteRoute="route('admin.vendor-users-management.roles.index')" :cancelText="trans('roles.cancel')" :deleteText="trans('roles.delete_role')" />
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            let table = $('#rolesDataTable').DataTable({
                ...
                ajax: {
                    url: '{{ route('admin.vendor-users-management.roles.data') }}',
                    type: 'GET',
                    data: function(d) {
                ...
                columns: [
                    ...
                    // Actions column
                    {
                        ...
                        render: function(data, type, row) {
                            let viewUrl =
                                "{{ route('admin.vendor-users-management.roles.show', ':id') }}"
                                .replace(':id', row.id);
                            let editUrl =
                                "{{ route('admin.vendor-users-management.roles.edit', ':id') }}"
                                .replace(':id', row.id);
