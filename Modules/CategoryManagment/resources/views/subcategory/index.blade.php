...
@section('content')
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body">
                                <div class="row g-3 align-items-end">
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <input type="text" 
                                                       ...
                                                       id="search" 
                                                       placeholder="{{ trans('categorymanagment::subcategory.search_by_name') }}"
                                                       autocomplete="off">
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label for="category_filter" class="il-gray fs-14 fw-500 mb-10">{{ trans('categorymanagment::subcategory.category') }}</label>
                                                <select class="form-control ih-medium ip-gray radius-xs b-light px-15" 
                                                        id="category_filter">
                                                    <option value="">{{ trans('categorymanagment::subcategory.all') }}</option>
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                <label for="active" class="il-gray fs-14 fw-500 mb-10">{{ trans('categorymanagment::subcategory.activation') }}</label>
                                                <select class="form-control ih-medium ip-gray radius-xs b-light px-15" 
                                                        id="active">
                                                    <option value="">{{ trans('categorymanagment::subcategory.all') }}</option>
                                                    <option value="1">{{ trans('categorymanagment::subcategory.active') }}</option>
                                                    <option value="0">{{ trans('categorymanagment::subcategory.inactive') }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                        ...
                                        <div class="col-md-1">
                                            <div class="form-group">
                                                ...
                                                <button type="button" id="exportExcel" class="btn btn-success btn-default btn-squared w-100" title="{{ trans('common.excel') }}">
                                                    <i class="uil uil-file-download-alt"></i> <span class="d-none d-lg-inline">{{ trans('common.excel') }}</span>
                                                </button>
                                            </div>
                                        </div>
                                </div>
                        ...
                        <table id="subcategoriesDataTable" class="table mb-0 table-bordered table-hover" style="width:100%">
                            <thead>
                                <tr class="userDatatable-header">
                                    ...
                                    @endforeach
                                    <th>
                                        <span class="userDatatable-title">{{ trans('categorymanagment::subcategory.category') }}</span>
                                    </th>
                                    <th>
                                        <span class="userDatatable-title">{{ trans('categorymanagment::subcategory.activation') }}</span>
                                    </th>
                                    <th>
                                        <span class="userDatatable-title">{{ trans('categorymanagment::subcategory.created_at') }}</span>
                                    </th>
                                    <th>
    ...
    <x-delete-with-loading
        modalId="modal-delete-subcategory"
        tableId="subcategoriesDataTable"
        deleteButtonClass="delete-subcategory"
        :title="__('main.confirm delete')"
        :message="__('main.are you sure you want to delete this')"
        itemNameId="delete-subcategory-name"
        confirmBtnId="confirmDeleteSubCategoryBtn"
            ...
            columns: [
                ...
                @endforeach
                {
                    ...
                    render: function(data) {
                        if (data && data.name) {
                            return '<span class="badge badge-round badge-primary badge-lg" data-category-id="' + data.id + '">' + data.name + '</span>';
                        }
                        return '-';
                    }
                },
                ...
                { data: 'created_at', name: 'created_at' },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        const showUrl = '{{ url("admin/category-management/subcategories") }}/' + row.id;
                        const editUrl = '{{ url("admin/category-management/subcategories") }}/' + row.id + '/edit';
                        const destroyUrl = '{{ url("admin/category-management/subcategories") }}/' + row.id;
