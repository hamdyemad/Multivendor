...
<div class="multi-select-wrapper" id="{{ $componentId }}-wrapper">
    @if ($label)
    ...
    <div class="multi-select-container" id="{{ $componentId }}" data-name="{{ $name }}">
        <div class="multi-select-display" id="{{ $componentId }}-display">
            <div class="multi-select-tags" id="{{ $componentId }}-tags">
                @if (count($selectedIds) > 1)
                    <span class="multi-select-count">{{ count($selectedIds) }} selected</span>
                @elseif (count($selectedIds) === 1)
                    @foreach ($options as $option)
                        @if (in_array((string) $option['id'], $selectedIds))
                            <span class="multi-select-tag" data-value="{{ $option['id'] }}">
                                {{ $option['name'] }}
                                <span class="multi-select-tag-remove" data-value="{{ $option['id'] }}">&times;</span>
                            </span>
                        @endif
                    @endforeach
                @endif
            </div>
            <input type="text" class="multi-select-search" id="{{ $componentId }}-search" 
                   placeholder="{{ count($selectedIds) > 0 ? '' : $placeholder }}" data-placeholder="{{ $placeholder }}" autocomplete="off">
            <span class="multi-select-arrow">
                <i class="uil uil-angle-down"></i>
            </span>
        </div>
        
        <div class="multi-select-dropdown" id="{{ $componentId }}-dropdown">
            @foreach ($options as $option)
                <div class="multi-select-option {{ in_array((string) $option['id'], $selectedIds) ? 'selected' : '' }}" 
                     data-value="{{ $option['id'] }}" data-text="{{ $option['name'] }}">
                    <span class="multi-select-checkbox">
                        <i class="uil {{ in_array((string) $option['id'], $selectedIds) ? 'uil-check' : '' }}"></i>
                    </span>
                    {{ $option['name'] }}
                </div>
            @endforeach
        ...
        <div class="multi-select-values" id="{{ $componentId }}-values">
            @foreach ($options as $option)
                @if (in_array((string) $option['id'], $selectedIds))
