<script>
    (function($) {
        ...
        const translations = {
            ...
            no_results_found: '{{ __('catalogmanagement::product.no_results_found') }}',
        };

        // Select2 language configuration
        const select2Language = {
            noResults: function() {
                return translations.no_results_found;
            }
        };
        ...
        function searchProducts(searchTerm) {
            ...
            // Clear previous product data (hides Step 3) when searching or clearing search
            clearPreviousProductData();

            if (!searchTerm || searchTerm.length < 2) {
                $('#products-list').html(
                    '<div class="col-12 text-center py-4"><p class="text-muted">{{ trans('catalogmanagement::product.enter_product_name_to_search') }}</p></div>'
                );
                return;
            }
            ...
            if (!selectedVendorId) {
                ...
                $('#products-list').html(
                    `<div class="col-12 text-center py-4"><p class="text-danger">{{ trans('catalogmanagement::product.please_select_vendor_first') }}</p></div>`
                );
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    console.log('📊 Response structure check:', {
                        ...
                        productsLength: response.data && response.data.products ? response.data
                            .products.length : 0,
                    ...
                    if (response.success && response.data && response.data.products && response.data
                        .products.length > 0) {
                        ...
                        $('#products-list').html(
                            '<div class="col-12 text-center py-4"><p class="text-muted">{{ __('catalogmanagement::product.no_available_products') }}</p></div>'
                        );
                    }
                    $('#products-list').show();
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#products-loading').hide();
                    $('#products-list').html(
                        `<div class="col-12 text-center py-4"><p class="text-danger">{{ trans('catalogmanagement::product.error_loading_products') }}: ${error}</p></div>`
                    ).show();
                }
            });
        }
                        ...
                        <div class="d-flex align-items-start">
                            ...
                            <div class="product-info flex-grow-1">
                                ...
                                <div class="product-details">
                                    ${product.brand ? `<small class="text-muted d-block"><i class="uil uil-tag-alt me-1"></i><strong>{{ __('catalogmanagement::product.brand') }}:</strong> ${product.brand}</small>` : ''}
                                    ${product.department ? `<small class="text-muted d-block"><i class="uil uil-building me-1"></i><strong>{{ __('catalogmanagement::product.department') }}:</strong> ${product.department}</small>` : ''}
                                    ${product.category ? `<small class="text-muted d-block"><i class="uil uil-layer-group me-1"></i><strong>{{ __('catalogmanagement::product.category') }}:</strong> ${product.category}</small>` : ''}
                                    ${product.sub_category ? `<small class="text-muted d-block"><i class="uil uil-sitemap me-1"></i><strong>{{ __('catalogmanagement::product.sub_category') }}:</strong> ${product.sub_category}</small>` : ''}
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        function loadProductVariants(productId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#variants-loading').hide();
                    if (response.success && response.data && response.data.variants && response.data
                        .variants.length > 0) {
                ...
                error: function() {
                    ...
                    $('#variants-container').html(
                        `<div class="alert alert-danger">{{ trans('catalogmanagement::product.error_loading_variants') }}</div>`
                    );
                }
            });
        }
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <form id="simple-product-form">
                                    ...
                                    <input type="hidden" name="vendor_product_variant_id" value="${product.variants && product.variants.length > 0 ? product.variants[0].id : ''}"">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                <label class="form-label">${translations.has_discount}</label>
        ...
        function displayVariantsFromSearch(product) {
            ...
            product.variants.forEach(function(variant, index) {
                ...
                html += createVariantManagementBox(variant, index, variantTreeName);
            });
        ...
        function createVariantManagementBox(variant, index, customTitle = null) {
            const keyName = variant.key?.name || 'Variant';
            const variantName = variant.name || `Variant ${variant.id}`;
            const displayTitle = customTitle ?
                `<strong>${keyName}:</strong> ${customTitle}` :
                (variant.key ? `<strong>${keyName}:</strong> ${variantName}` : variantName);

            // Determine if this is a System Variant (from search) or Vendor Variant (existing)
            const isSystemVariant = !!variant.variant_tree;

            // Logic to handle IDs:
            // For System Variant (Search): id is the ProductVariant ID.
            // For Vendor Variant (Existing): id is the VendorProductVariant ID.

            // We need to send 'product_variant_id' so the backend can find the system variant SKU if needed (User Request)
            const productVariantId = isSystemVariant ? variant.id : (variant.product_variant_id || '');

            // For saving constraints: 
            // 'id' usually maps to the primary key of the record being edited (VendorProductVariant). Empty if new.
            const recordId = !isSystemVariant ? variant.id : '';

            // 'vendor_product_variant_id' seems to be used by backend/user logic to link/lookup. 
            // If new (System Variant), we send system ID here too (as user requested "fix because sent with null").
            // If existing, we send the vendor variant ID.
            const vendorProductVariantId = isSystemVariant ? variant.id : variant.id;
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form class="variant-form" data-variant-id="${variant.id}">
                            <input type="hidden" name="variants[${index}][id]" value="${recordId}">
                            <input type="hidden" name="variants[${index}][product_variant_id]" value="${productVariantId}">
                            <input type="hidden" name="variants[${index}][vendor_product_variant_id]" value="${vendorProductVariantId}">
        ...
        function initEventHandlers() {
            ...
            $(document).on('change', 'select[name*="[region_id]"]', function() {
                ...
                if ($select.val()) {
                    ...
                    let $targetElement = $select;
                    if ($select.hasClass('select2-hidden-accessible')) {
                        $targetElement = $select.next('.select2-container');
                    }
                    $targetElement.next('.error-message').hide();
                }
            });
        }
        ...
        function addStockRow(variantId) {
            ...
            $(`#variant-${variantId}-stock-rows tr:last .region-select`).select2({
                theme: 'bootstrap-5',
                width: '100%'
            });
            populateRegionOptions();
        ...
        function validateRequiredFields() {
            ...
            if (!maxPerOrder || parseInt(maxPerOrder) < 1) {
                ...
                $('#error-max_per_order').text('{{ trans('catalogmanagement::product.max_per_order_required') }}')
                    .show();
                isValid = false;
            } else {
            ...
            $('.variant-management-box, .existing-variant-card').each(function(index) {
                const $variant = $(this);
                const variantName = $variant.find('h6 .variant-name').text() || $variant.find('h6')
                ...
                if (!price || parseFloat(price) <= 0) {
                    errors.push(
                        `${variantName}: {{ trans('catalogmanagement::product.price_greater_than_zero') }}`
                    );
                    ...
                    if ($errorElement.length === 0) {
                        $errorElement = $(
                            '<div class="error-message text-danger" style="font-size: 0.875rem; margin-top: 0.25rem;"></div>'
                        );
                        $priceInput.after($errorElement);
                    }
                    $errorElement.text('{{ trans('catalogmanagement::product.price_greater_than_zero') }}')
                        .show();
                    isValid = false;
                } else {
                ...
                stockRows.each(function() {
                    ...
                    if (!region || region === '') {
                        ...
                        stockErrors.push(
                            '{{ trans('catalogmanagement::product.region_required') }}');
                        // Add to main errors array
                        errors.push(
                            `${variantName}: {{ trans('catalogmanagement::product.region_required') }}`
                        );

                        // Show error message (handling Select2)
                        let $targetElement = $regionSelect;
                        if ($regionSelect.hasClass('select2-hidden-accessible')) {
                            $targetElement = $regionSelect.next('.select2-container');
                        }

                        let $errorElement = $targetElement.next('.error-message');
                        if ($errorElement.length === 0) {
                            $errorElement = $(
                                '<div class="error-message text-danger" style="font-size: 0.875rem; margin-top: 0.25rem;"></div>'
                            );
                        ...
                        $errorElement.text(
                            '{{ trans('catalogmanagement::product.region_required') }}').show();
                    } else {
                    ...
                    if (!quantity || parseInt(quantity) < 0) {
                        ...
                        $errorElement.text(
                                '{{ trans('catalogmanagement::product.quantity_min_zero') }}')
                            .show();
                        stockErrors.push(
                            '{{ trans('catalogmanagement::product.quantity_required') }}');
                    } else {
                ...
                if (!hasValidStock && stockRows.length > 0) {
                    errors.push(
                        `${variantName}: {{ trans('catalogmanagement::product.at_least_one_valid_stock') }}`
                    );
                    isValid = false;
                } else if (stockRows.length === 0) {
                    errors.push(
                        `${variantName}: {{ trans('catalogmanagement::product.at_least_one_stock') }}`);
                    isValid = false;
                }
            });
            ...
            console.log('  - Variant elements processed:', $('.variant-management-box, .existing-variant-card')
                .length);

            // Show validation errors
            if (!isValid) {
                const errorMessage = errors.join('<br>');
                showBootstrapAlert('danger',
                    `{{ __('catalogmanagement::product.please_fix_validation_errors') }}<br>${errorMessage}`);
        ...
        function saveForm() {
            ...
            saveButton.html(
                '<i class="spinner-border spinner-border-sm me-1"></i> {{ __('common.saving') ?? 'Saving...' }}'
            );
        ...
        function highlightFieldError(field, errorMessage) {
                ...
                if (matches) {
                    ...
                    if (variantField === 'price') {
                        ...
                        if (stockMatches) {
                            ...
                            if (stockField === 'region_id') {
                                $input = $(
                                    `select[name="variants[${variantIndex}][stocks][${stockIndex}][region_id]"]`
                                );
                            } else if (stockField === 'quantity') {
                                $input = $(
                                    `input[name="variants[${variantIndex}][stocks][${stockIndex}][quantity]"]`);
                            } else if (stockField === 'alert_quantity') {
                                $input = $(
                                    `input[name="variants[${variantIndex}][stocks][${stockIndex}][alert_quantity]"]`
                                );
                            }
                        }
                    }
            ...
            if ($input && $input.length > 0) {
                ...
                if ($container.hasClass('input-group') || $container.hasClass('form-control-wrapper') || $container
                    .hasClass('select2-selection')) {
            ...
            if (selectedVendorId) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (selectedVendorId && regions.length === 0) {
                        ...
                        console.log('✅ Region options populated for vendor:', selectedVendorId, regions
                            .length);
                    },
                    error: function(xhr, status, error) {
        ...
        `;
            ...
            stockContainer.find('.select2').last().select2({
                theme: 'bootstrap-5',
                width: '100%'
            });
        ...
        $(document).on('change', '.variant-discount-toggle', function() {
            const productIndex = $(this).closest('.variant-box').find('input[name*="[variant_key_id]"]')
                .attr('name').match(/\[(\d+)\]/)[1];
        ...
        $(document).on('change', '[id^="variant_key_"]', function() {
            ...
            if (keyId) {
                ...
                const selectVariantValueText =
                    '{{ __('catalogmanagement::product.select_variant_value') }}';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && response.configuration) {
                            $(`#variant_config_id_${productIndex}_${variantIndex}`).val(response
                                .configuration.id);
                        }
                    },
                    error: function(xhr) {
        ...
        function validateForm() {
            ...
            if (!points || points < 0) {
                ...
                vendorForm.find('#points').next('.invalid-feedback').text(
                    '{{ __('catalogmanagement::product.points_required') }}');
            ...
            if (!maxPerOrder || maxPerOrder < 1) {
                ...
                vendorForm.find('#max_per_order').next('.invalid-feedback').text(
                    '{{ __('catalogmanagement::product.max_per_order_required') }}');
            ...
            if (productType === 'simple') {
                ...
                if (!sku || sku.trim() === '') {
                    ...
                    $('input[name="sku"]').siblings('.invalid-feedback').text(
                        '{{ __('catalogmanagement::product.sku_required') }}');
                ...
                if (!price || price <= 0) {
                    ...
                    $('input[name="price"]').siblings('.invalid-feedback').text(
                        '{{ __('catalogmanagement::product.price_required') }}');
                ...
                if ($('input[name="has_discount"]').is(':checked')) {
                    ...
                    if (!priceBeforeDiscount || priceBeforeDiscount <= 0) {
                        ...
                        $('input[name="price_before_discount"]').siblings('.invalid-feedback').text(
                            '{{ __('catalogmanagement::product.price_before_discount_required') }}');
                    ...
                    if (!offerEndDate) {
                        ...
                        $('input[name="offer_end_date"]').siblings('.invalid-feedback').text(
                            '{{ __('catalogmanagement::product.offer_end_date_required') }}');
                ...
                $('.stock-row').each(function() {
                    const row = $(this);
                    const regionId = row.find('select[name="region_id"], select[name*="[region_id]"]')
                        .val();
                    const quantity = row.find('input[name="stock"], input[name*="[stock]"]').val();

                    if (!regionId) {
                        row.find('select[name="region_id"], select[name*="[region_id]"]').addClass(
                            'is-invalid');
                        row.find('select[name="region_id"], select[name*="[region_id]"]').siblings(
                            '.invalid-feedback').text(
                            '{{ __('catalogmanagement::product.region_required') }}');
                    ...
                    if (!quantity || quantity < 0) {
                        ...
                        row.find('input[name="stock"], input[name*="[stock]"]').siblings(
                            '.invalid-feedback').text(
                            '{{ __('catalogmanagement::product.stock_required') }}');
                ...
                $('.variant-box').each(function() {
                    ...
                    variantBox.find('.stock-row').each(function() {
                        ...
                        if (!quantity || quantity < 0) {
                            ...
                            errors.push(
                                '{{ __('catalogmanagement::product.quantity_required') }}');
                            isValid = false;
                        }
                    });
                });
        ...
        function saveVendorProducts() {
            ...
            if (!selectedProductsData || selectedProductsData.length === 0) {
                showBootstrapAlert('warning',
                    '{{ __('catalogmanagement::product.please_select_products_first') }}');
                return;
            }
            ...
            if (!selectedVendorId) {
                showBootstrapAlert('warning',
                    '{{ __('catalogmanagement::product.please_select_vendor_first') }}');
                return;
            }
        ...
        `);
            ...
            $.ajax({
                    ...
                    if (response.success) {
                        if (typeof toastr !== 'undefined') {
                            toastr.success(response.message ||
                                '{{ __('catalogmanagement::product.vendor_products_saved_successfully') }}'
                            );
                        }
                ...
                error: function(xhr) {
                    ...
                    let errorMessage =
                        '{{ __('catalogmanagement::product.error_saving_bank_stock') }}';
                    let detailedErrors = [];
        ...
        function loadRegions() {
            ...
            return $.ajax({
                ...
                success: function(response) {
                    ...
                    // Check if vendor has no regions and show alert
                    if (selectedVendorId && regionsData.length === 0) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                icon: 'warning',
                                title: '{{ __('common.warning') }}',
        ...
        function addVariantBox() {
            ...
            if (!template) {
                console.error(
                    '❌ Variant box template not found! Make sure #variant-box-template exists in the DOM.');
                return;
            }
            ...
            $('.tax-select').each(function() {
                ...
                if (taxes && taxes.length > 0) {
                    taxes.forEach(tax => {
                        ...
                        taxSelect.append(
                            `<option value="${tax.id}">${taxName} (${taxRate}%)</option>`);
                    });
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showAlert('Success', 'Product stock saved successfully!',
                                'success');
                            ...
                            showAlert('Error', response.message || 'Error saving stock',
                                'error');
                        }
                    },
                    error: function(xhr, status, error) {
        ...
        function testValidationErrors() {
            ...
            console.log(
                '✅ Test validation errors displayed. Check the form for red borders and error messages under inputs.'
            );
        }
